...
export default function Game() {
  ...
  const drawCard = () => {
    if (remainingQuestions.length === 0) {
      setIsGameOver(true);
      setCurrentQuestion('Congratulations! You have completed all the questions.');
      return;
    }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#FFF8E7] p-4">
      <div className={`w-64 h-96 rounded-2xl shadow-2xl flex items-center justify-center p-6 transform transition-all duration-300 ease-in-out hover:scale-105 ${isOrangeCard ? 'bg-[#F9A60F] text-white' : 'bg-[#6BC7E9] text-white'}`}>
        <p className={`text-center ${playfair.className} text-lg`}>{currentQuestion}</p>
      </div>
      <div className="mt-12 flex space-x-6">
        <Button 
          onClick={drawCard} 
          className={`${montserrat.className} text-lg px-8 py-3 rounded-full transition-all duration-300 w-36 ${isOrangeCard ? 'bg-[#6BC7E9] hover:bg-[#5AB7D9] text-white' : 'bg-[#F9A60F] hover:bg-[#E99600] text-white'}`} 
          disabled={isGameOver}
        >
          Draw
        </Button>
        <Button 
          ...
          className={`${montserrat.className} text-lg px-8 py-3 rounded-full transition-all duration-300 w-36 ${isOrangeCard ? 'bg-[#6BC7E9] hover:bg-[#5AB7D9] text-white' : 'bg-[#F9A60F] hover:bg-[#E99600] text-white'}`}
        >
          <Shuffle className="mr-2 h-5 w-5" /> Restart
        </Button>
      </div>
    </div>
  );
